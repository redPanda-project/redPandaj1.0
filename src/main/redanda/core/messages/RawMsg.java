...
import main.redanda.crypt.AESCrypt;
import main.redanda.crypt.ECKey;
import main.redanda.crypt.ECKey.ECDSASignature;
...
public class RawMsg implements Serializable, Comparable<RawMsg> {
    ...
    public RawMsg toSpecificMsgType() {
        ...
        if (decryptedContent[0] == ImageMsg.BYTE) {
            ...
            return imageMsg;
        } else //Check for InfoMsg
        if (decryptedContent[0] == InfoMsg.BYTE) {
            InfoMsg infoMsg = new InfoMsg(key, timestamp, nonce, signature, content, decryptedContent, channel, verified, readable, database_Id);
            return infoMsg;
        }
