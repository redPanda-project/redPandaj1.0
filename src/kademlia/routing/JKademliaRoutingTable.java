...
import kademlia.node.KeyComparatorContacts;
...
public class JKademliaRoutingTable implements KademliaRoutingTable {
    ...
    public synchronized final List<Node> findClosest(KademliaId target, int numNodesRequired) {
        TreeSet<Contact> sortedSet = new TreeSet<Contact>(new KeyComparatorContacts(target));
        sortedSet.addAll(this.getAllContacts());

        List<Node> closest = new ArrayList<>(numNodesRequired);

        /* Now we have the sorted set, lets get the top numRequired */
        int count = 0;
        for (Contact c : sortedSet) {
            if (c.staleCount() > 0) {
//                System.out.println("stale node: " + c.getNode().getNodeId());
                //node is stale, so we dont count him as a clostest node to target!
                continue;
            }
